#!/usr/bin/env bash
# run_faers_uniform_strong_full.sh
#
# Single-job matched run: FAERS workload, uniform_strong policy, full 50,000-record dataset.
#
# Purpose: produce a matched FAERS strong result using the same record count,
# iteration count, warmup count, and scheme as run_faers_uniform_baseline_full.sh
...
POLICY_MODE="uniform_strong"
PAYLOAD_SOURCE="faers"
LABEL="faers_uniform_strong"
...
RUN_DIR="${SCRIPT_DIR}/results/faers_full_matched_strong/${TIMESTAMP}"
...
FAERS Uniform Strong — Full Dataset Matched Run
================================================
purpose               : matched full-dataset comparison (strong leg)
timestamp             : ${TIMESTAMP}
run_dir               : ${RUN_DIR}
...
----------------------
  ...
  Matched baseline run:
    results/faers_full_matched_baseline/<timestamp>/json/faers_uniform_baseline.json
...
log "=== FAERS Uniform Strong Full-Dataset Runner ==="
